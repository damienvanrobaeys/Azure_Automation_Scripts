...
# Folder where to send the CSV on SharePoint
$CSV_Folder_Location = ""

# URL of the CSV that lists top 20 BSOD
$Exported_CSV_URL = ""
...
ForEach($BSOD in $Get_All_BSOD)
	{
		...
		Add-Member -InputObject $BSOD_Obj -MemberType NoteProperty -Name "Log file date" -Value $Log_File_Date						
		$BSOD_Array += $BSOD_Obj	     
	}
...
Connect-PnPOnline -Url $Site_URL -ClientId $ClientID -ClientSecret $Secret -WarningAction Ignore
Add-PnPFile -Path $CSV_Name -Folder $CSV_Folder_Location | out-null
