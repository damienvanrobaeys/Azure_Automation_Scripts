...
# info about the location of BSOD log files
# It should be something like /sites/<your site>/path of logs folder 
$BSOD_Folder_Location = ""

# Teams webhoot link
$Webhook_URL = ""

# Teams notif design
$Notif_Title_Info = "Top 20 devices with BSOD (info)"
$Notif_Title_Logs = "Top 20 devices with BSOD (logs)"

$Notif_Message = "Here is the list of top 20 devices with BSOD on the last 30 days"
$Color = "2874A6"
#*****************************************************************

# URL of logs to display in the report
$BSOD_Log_File_Path = "$Site_URL/$BSOD_Folder_Location"
...
# Graph URL to use
$Top20_BSOD_URL = "https://graph.microsoft.com/beta/deviceManagement/userExperienceAnalyticsDevicePerformance?dtFilter=all&`$orderBy=blueScreenCount%20desc&`$top=20&`$filter=blueScreenCount%20ge%201%20and%20blueScreenCount%20le%2050"

$All_BSOD = Invoke-WebRequest -Uri $Top20_BSOD_URL -Method GET -Headers $Headers -UseBasicParsing 
...
ForEach($BSOD in $Get_All_BSOD)
	{
		...
		$BSOD_Log_File = "$BSOD_Folder_Location/$BSOD_File_Name"		
		$Get_Log_File = Get-PnPFile -Url $BSOD_Log_File -ea SilentlyContinue
		If($Get_Log_File -ne $null)
			{
				$Log_File_Link = "$BSOD_Log_File_Path/$BSOD_File_Name"   
			}
		Else 
			{
		...
		Add-Member -InputObject $BSOD_Obj -MemberType NoteProperty -Name "Last code info" -Value $Get_Last_Error_Label
		Add-Member -InputObject $BSOD_Obj -MemberType NoteProperty -Name "Log file" -Value $Log_File_Link
		$BSOD_Array += $BSOD_Obj	     
	}

$Main_Array = $BSOD_Array | Select Device, Model, Count, "Last BSOD", "Last code", "Last code info"
$Log_Array = $BSOD_Array | Select Device, "Log file"

# $NewFile = New-Item -ItemType File -Name $CSV_Name
# $BSOD_Array | select * | export-csv $CSV_Name -notype -Delimiter ";"

$Main_Array = $BSOD_Array | Select Device, Model, Count, "Last BSOD", "Last code", "Last code info"
$Log_Array = $BSOD_Array | Select Device, Model, "Log file"

$Main_BSOD_Table = $Main_Array | ConvertTo-HTML -Fragment
$Main_BSOD_Table = $Main_BSOD_Table.Replace("<table>","<table border='1'>")

$BSOD_Logs_Table = $Log_Array | ConvertTo-HTML -Fragment
$BSOD_Logs_Table = $BSOD_Logs_Table.Replace("<table>","<table border='1'>")

$Text_Message = "$Notif_Message<br><br>
$Main_BSOD_Table
"
Send_Notif -Text $Text_Message -Title $Notif_Title_Info  | out-null

$Text_Message = "$Notif_Message<br><br>
$BSOD_Logs_Table
"
Send_Notif -Text $Text_Message -Title $Notif_Title_Logs  | out-null
