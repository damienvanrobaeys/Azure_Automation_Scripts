...
Invoke-WebRequest -Method POST -Uri $URL -Headers $Headers -UseBasicParsing -Body $GroupMember -ContentType 'application/json'     
                    ...
                    {
                        $URL = "https://graph.microsoft.com/v1.0/groups/$Target_Group_ID/members/$Device_ObjectID/`$ref"
                        $URL
                        Invoke-WebRequest -Method DELETE -Uri $URL -Headers $Headers -UseBasicParsing   
                    }                                                        
                }
        }    
}